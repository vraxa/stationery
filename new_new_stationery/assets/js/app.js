    ...
    .map((item) => {
      ...
      <div class="item-row${inCart ? " in-cart" : ""}" data-id="${esc(item.id)}">
        <div class="item-row-info">
          ...
          <div class="item-desc">${item.sku ? `<span class="item-sku">${esc(item.sku)}</span>` : ""}${item.description ? esc(item.description) : ""}</div>
        </div>
...
document.getElementById("cart-items").addEventListener("click", (e) => {
  ...
  if (btn) {
    ...
    setQty(
      id,
      btn.dataset.cartAction === "inc" ? cur + 1 : Math.max(1, cur - 1),
    );
  }
});
...
document.getElementById("btn-order").addEventListener("click", async () => {
  ...
  const cart = Object.entries(state.cart).map(([id, qty]) => ({
    id,
    quantity: qty,
  }));
