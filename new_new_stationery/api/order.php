...
if (empty($cart)) {
    http_response_code(422); echo json_encode(['error' => 'Cart is empty.']); exit;
}
if ($deptId === '') {
    http_response_code(422); echo json_encode(['error' => 'Please select a department.']); exit;
}

// Load department
$departments = json_decode(file_get_contents(dirname(__DIR__) . '/data/departments.json'), true) ?? [];
$department  = null;
foreach ($departments as $d) {
    if ($d['id'] === $deptId) { $department = $d; break; }
}
if ($department === null) {
    http_response_code(422); echo json_encode(['error' => 'Selected department not found.']); exit;
}

// Validate cart items against live data (prevent price tampering)
$allItems  = json_decode(file_get_contents(dirname(__DIR__) . '/data/items.json'), true) ?? [];
$itemIndex = [];
foreach ($allItems as $item) { $itemIndex[$item['id']] = $item; }
...
foreach ($cart as $entry) {
    ...
    if ($qty <= 0) continue;
    if (!isset($itemIndex[$id])) continue;
...
if (empty($validatedCart)) {
    http_response_code(422); echo json_encode(['error' => 'No valid items in cart.']); exit;
}
...
foreach ($validatedCart as $row) {
    $rowsHtml .= sprintf(
        ...
        $currency, number_format($row['subtotal'], 2)
    );
}

$notesSection = '';
if ($orderNotes !== '') {
    $notesSection = '<p style="margin-top:16px;"><strong>Additional Notes:</strong><br>'
        . nl2br(htmlspecialchars($orderNotes)) . '</p>';
}

$userDept    = $user['department'] !== '' ? ' (' . htmlspecialchars($user['department']) . ')' : '';
$userTitle   = $user['job_title']  !== '' ? htmlspecialchars($user['job_title'])         : '';
...
<body style="font-family:'Segoe UI',Calibri,Arial,sans-serif;color:#323130;margin:0;padding:0;background:#F3F2F1;">
  <div style="max-width:640px;margin:24px auto;background:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.1);">
    <div style="background:#0078D4;padding:24px 32px;">
      ...
      <p style="margin:4px 0 0;color:#deecf9;font-size:14px;">{$appConfig['system_name']}</p>
    </div>
    <div style="padding:24px 32px;">
      ...
      <h2 style="font-size:16px;font-weight:600;border-bottom:2px solid #0078D4;padding-bottom:8px;margin-top:24px;">Order Summary</h2>
      <table style="width:100%;border-collapse:collapse;font-size:14px;">
        ...
        <tfoot>
          <tr style="background:#F3F2F1;">
            ...
            <td style="padding:10px 12px;font-weight:700;text-align:right;color:#0078D4;">{$currency}{$total}</td>
          </tr>
        </tfoot>
      </table>
...
if ($token === 'DEV_MODE') {
    echo json_encode([
        'success' => true,
        'message' => "[DEV] Email simulated — would send to {$department['head_name']} ({$department['email']}).",
    ]);
    exit;
}

$sent = graphSendMail($token, $department['email'], $department['head_name'], $subject, $htmlBody);

if (!$sent) {
    http_response_code(502);
    echo json_encode(['error' => 'Failed to send email. Please try again or contact IT.']);
    exit;
}

echo json_encode(['success' => true, 'message' => "Order sent to {$department['head_name']} at {$department['email']}."]);
